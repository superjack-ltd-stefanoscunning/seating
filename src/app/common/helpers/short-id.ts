...
// Copyright 2024 the Short ID authors. All rights reserved. Apache 2.0 license.

// @ts-ignore

const version = environment.version;

export interface ShortIdRanges {
  [k: string]: [number, number];
}

export interface ShortIdRangesMap {
  [k: string]: ShortIdRanges;
}

export type ShortIdDefaultDictionaries =
  | 'number'
  | 'alpha'
  | 'alpha_lower'
  | 'alpha_upper'
...
export interface ShortIdOptions {
  ...
  dictionary: string[] | ShortIdDefaultDictionaries;
...
export const DEFAULT_OPTIONS: ShortIdOptions = {
  dictionary: 'alphanum',
  shuffle: true,
  debug: false,
 ...
 * const options = { ... };
 *
 * const uid = new ShortId(options);
 * ```
 *
 * For more information take a look at the [ShortIdOptions type definition](/interfaces/shortidoptions.html).
 */
export default class ShortId {
  /** @hidden */
  static default: typeof ShortId = ShortId;
  ...
  protected _number_dict_ranges: ShortIdRanges = {
    digits: [this._digit_first_ascii, this._digit_last_ascii],
  };

  protected _alpha_dict_ranges: ShortIdRanges = {
    lowerCase: [this._alpha_lower_first_ascii, this._alpha_lower_last_ascii],
    upperCase: [this._alpha_upper_first_ascii, this._alpha_upper_last_ascii],
  };

  protected _alpha_lower_dict_ranges: ShortIdRanges = {
    lowerCase: [this._alpha_lower_first_ascii, this._alpha_lower_last_ascii],
  };

  protected _alpha_upper_dict_ranges: ShortIdRanges = {
    upperCase: [this._alpha_upper_first_ascii, this._alpha_upper_last_ascii],
  };

  protected _alphanum_dict_ranges: ShortIdRanges = {
    digits: [this._digit_first_ascii, this._digit_last_ascii],
    lowerCase: [this._alpha_lower_first_ascii, this._alpha_lower_last_ascii],
    upperCase: [this._alpha_upper_first_ascii, this._alpha_upper_last_ascii],
  };

  protected _alphanum_lower_dict_ranges: ShortIdRanges = {
    digits: [this._digit_first_ascii, this._digit_last_ascii],
    lowerCase: [this._alpha_lower_first_ascii, this._alpha_lower_last_ascii],
  };

  protected _alphanum_upper_dict_ranges: ShortIdRanges = {
    digits: [this._digit_first_ascii, this._digit_last_ascii],
    upperCase: [this._alpha_upper_first_ascii, this._alpha_upper_last_ascii],
  };

  protected _hex_dict_ranges: ShortIdRanges = {
    decDigits: [this._digit_first_ascii, this._digit_last_ascii],
    alphaDigits: [this._alpha_lower_first_ascii, this._hex_last_ascii],
  };

  protected _dict_ranges: ShortIdRangesMap = {
  ...
  protected _normalizeDictionary = (
    ...
    if (dictionary && Array.isArray(dictionary) && dictionary.length > 1) {
      ...
      const rangesName = `_${
        dictionary as ShortIdDefaultDictionaries
      }_dict_ranges`;
      ...
      Object.keys(ranges).forEach((rangeType) => {
        ...
        for (
          ...
          this.lowerBound <= this.upperBound
            ...
            : i > this.upperBound;
          this.dictIndex =
            this.lowerBound <= this.upperBound ? (i += 1) : (i -= 1)
        ) {
   ...
   * @alias `const uid = new ShortId(); uid.seq();`
   */
   ...
   * @alias `const uid = new ShortId(); uid.rnd(uuidLength: number);`
   */
  randomUUID = (
    uuidLength: number = this.uuidLength || DEFAULT_UUID_LENGTH
  ): string => {
   ...
   * @alias `const uid = new ShortId(); uid.fmt(format: string);`
   */
  ...
  stamp = (finalLength: number, date?: Date): string => {
    ...
    return `${id.substring(0, rndIdx)}${hexStamp}${id.substring(
      rndIdx
    )}${rndIdx.toString(16)}`;
  };
  ...
  constructor(argOptions: Partial<ShortIdOptions> = {}) {
    const options: ShortIdOptions = {
      ...DEFAULT_OPTIONS,
      ...(argOptions as Partial<ShortIdOptions>),
    };
